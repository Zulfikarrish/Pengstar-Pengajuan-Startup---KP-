...
class UserController extends Controller
{
    ...
    public function storePengaduan(Request $request)
    {
        ...
        if ($request->file('foto')) {
            $data['foto'] = $request->file('foto')->store('images/pictures', 'public');
        }
    ...
    public function laporan($siapa = '')
    {
        ...
        $gagal = Pengaduan::where([['nik', Auth::guard('masyarakat')->user()->nik], ['status', 'gagal']])->get()->count();
        // Membuat variable $terverifikasi isinya menghitung pengaduan status selesai
        $lulus = Pengaduan::where([['nik', Auth::guard('masyarakat')->user()->nik], ['status', 'lulus']])->get()->count();

        // Masukkan 3 variable diatas ke dalam variable array $hitung
        $hitung = [$terverifikasi, $gagal, $lulus];
